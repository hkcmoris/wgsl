...
// import {
//     PHYSICS_COLLISION_TYPE,
//     PhysicsObject,
//     addPhysics,
// } from "../render/physics/physics";
import {
...
class SphereStruct {
    ...
    constructor(object/*: SphereObject3D*/) {
        if (!(object instanceof SphereObject3D)) {
            throw new Error("Invalid object type. Object should be SphereObject3D.");
        }
...
const _generateRandomColor = () => {
    return RANDOM_COLOR_LIST[
        Math.floor(Math.random() * RANDOM_COLOR_LIST.length)
    ];
};
...
class RayMarchShader extends THREE.ShaderMaterial {
    ...
    constructor() {
        ...
        const nearPlaneHeight =
            2 * Math.tan((camera.fov * Math.PI) / 180 / 2) * camera.near; // height
        const nearPlaneWidth = nearPlaneHeight * camera.aspect; // width

        const farPlaneHeight =
            2 * Math.tan((camera.fov * Math.PI) / 180 / 2) * camera.far; // height
        const farPlaneWidth = farPlaneHeight * camera.aspect; // width

        const { width, height } = useRenderSize();

        const onRenderResize = (
            windowWidth /*: number*/,
            windowHeight /*: number*/
        ) => {
            if (this.shaderPass) {
                ...
                this.shaderPass.uniforms.uCameraAspectRatio.value =
                    camera.aspect;

                const nearPlaneWidth = nearPlaneHeight * camera.aspect; // width
                this.shaderPass.uniforms.uCameraNearSize.value.set(
                    nearPlaneWidth,
                    nearPlaneHeight
                );
            }
        ...
        this.uniforms = {
            ...
            uStickyDepthTexture: {
                value: this.stickyDepthRenderTarget.depthTexture,
            },
            ...
            uCameraNearSize: {
                value: new THREE.Vector2(nearPlaneWidth, nearPlaneHeight),
            },
            uCameraFarSize: {
                value: new THREE.Vector2(farPlaneWidth, farPlaneHeight),
            },
            ...
            // * lights
            ...generateLightShaderUniforms(
                DIR_LIGHTS_UNIFORM_NAME,
                AMBIENT_LIGHTS_UNIFORM_NAME
            ),
        };
            ...
            if (rigidBody) {
                rigidBody.applyImpulse(
                    direction.multiplyScalar(
                        object.radius * 20 * object.radius
                    ),
                    true
                );
            }
    ...
    update(
        ...
        if (this.shaderPass) {
            ...
            this.shaderPass.uniforms.uCameraPosition.value.copy(
                camera.position
            );
            this.shaderPass.uniforms.uCameraQuaternion.value.copy(
                camera.quaternion
            );
            ...
            this.shaderPass.uniforms.uCameraDirection.value.copy(
                camera.getWorldDirection(vec3_0)
            );
